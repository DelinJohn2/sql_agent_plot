...
class StepFilter(logging.Filter):
    def filter(self, record):
        if 'step' not in record.__dict__:
            ...
            # Ensure step is always a string, to avoid formatting errors
            record.step = str(record.step)  # Convert step to string if it is not
        return True

# Configure logging to include timestamps and step numbers for clarity and save logs to a file
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - STEP %(step)s - %(levelname)s - %(message)s',  # Use %s for step to avoid TypeError
...
try:
    ...
    raise e  # Re-raise exception to stop execution if DB connection fails
...
try:
    ...
    raise e  # Re-raise exception to stop execution if LLM initialization fails
...
def interactive_agent(agent):
    logger.info("Starting interactive session.", extra={'step': 11})
    while True:
        user_input = input("\nYou: ").strip()

        if user_input.lower() in ("exit", "quit"):
            logger.info("Exiting. Goodbye!", extra={'step': 11})
            break
